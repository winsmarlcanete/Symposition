...
public class ScoreScreenWaltz implements Screen {
    ...
    public ScoreScreenWaltz(final Symposition game){
        this.game = game;

        camera = new OrthographicCamera();
        camera.setToOrtho(false, 800, 480);

        bg = new Texture(Gdx.files.internal("bgImages/Waltz_Score.png"));
        skin = new Skin(Gdx.files.internal("rainbowui/rainbow-ui.json"));
        fontskin = new Skin(Gdx.files.internal("menu/label/regular.json"));

        music = Gdx.audio.newMusic(Gdx.files.internal("sounds/levelmusic/waltzfinal.wav"));
        music.play();
